...
class MovementReport:
    ...
    def __init__(self, mvmt_ob, global_params):
        ...
        self.report_dir = global_params.envt_params.envt['Report_Directory'] + mvmt_ob.obj_id + '/'
...
class EventReport:
    ...
    def __init__(self, event_ob, global_params):
        """ Initialize with a specific directory to the mvmt_ob """
        self.report_dir = global_params.envt_params.envt['Report_Directory'] + 'events' + '/'
        if not os.path.isdir(self.report_dir):
            os.makedirs(self.report_dir)
    ...
    def create_log(self, event_ob):
        f_ptr = open('%s%s_%d.dat' % (self.report_dir, self.event_id, shared.Controls.no_of_simulation), 'a')
        txt = 'EID: %s\nTime: %s\nSource: %s\nDuration: %s\nExpiry: %s\nExpired: %s\nData: %s\nBuffer: %s' % (self.event_id,
                                                                                                              simtimer.convert_HMS(event_ob.time),
                                                                                                              event_ob.src_obj,
