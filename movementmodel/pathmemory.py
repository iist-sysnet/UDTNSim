...
class PathMemoryMovement(pt.PathTypeMovement):
    ...
    def compute_next_node(self, global_params):
        ...
        # Select only the untraversed ways from the neighbor nodes
        untraversed_nodes = []
        for node in possible_nodes:
            if road_graph[self.next_node][node]['way_id'] not in self.ways_visited:
                untraversed_nodes.append(node)
                
        if untraversed_nodes:
            possible_nodes = untraversed_nodes[:]
